...
function ProgressToast({
    ...
    return (
        <div className="bg-background relative overflow-hidden rounded-md border shadow-sm">
            {/* Background progress that shrinks as time passes */}
            <div
                className={`pointer-events-none absolute inset-0 ${variantStyles[variant]}`}
                style={{
            ...
            <div className="relative z-10 p-4">
                <div className="flex items-start gap-3">
                    {Icon && (
                        <Icon
                            className={`h-5 w-5 ${iconColorStyles[variant]} shrink-0`}
                        />
                    )}
            ...
            <div className="bg-muted absolute right-0 bottom-0 left-0 h-[3px]">
                <div
                    className={progressBarStyles[variant]}
...
function AlertToast({
    ...
    return (
        <div className="bg-background rounded-md border p-4 shadow-sm">
            <div className="flex items-start gap-3">
                <Icon
                    className={`h-5 w-5 ${variantStyles[variant]} shrink-0`}
                />
                <div className="flex-1">
                    <div className="font-medium">{title}</div>
                    {description && (
...
export const toast: ToastAPI = {
    ...
    loading: (
        title: string,
        { description, position = 'bottom-right' }: LoadingToastOptions = {}
    ) => {
        return sonnerToast.custom(
